...
typedef struct {
    ...
    XSetup_joint_aie_uio_map maps[ MAX_UIO_MAPS ];
} XSetup_joint_aie_uio_info;

/***************** Variable Definitions **************************************/
static XSetup_joint_aie_uio_info uio_info;
...
static int uio_info_read_name(XSetup_joint_aie_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSetup_joint_aie_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSetup_joint_aie_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSetup_joint_aie_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSetup_joint_aie_Initialize(XSetup_joint_aie *InstancePtr, const char* InstanceName) {
	XSetup_joint_aie_uio_info *InfoPtr = &uio_info;
