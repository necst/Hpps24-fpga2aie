...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        printf("Usage: %s <rows> <columns> <max_grid>\n", argv[0]);
        return 1;
    }

    int rows = atoi(argv[1]);
    int columns = atoi(argv[2]);
    int max_histo_grid = atoi(argv[3]);
    ...
    uint8_t* img1_host = (uint8_t*) malloc(rows * columns * sizeof(uint8_t));
    uint8_t* img2_host = (uint8_t*) malloc(rows * columns * sizeof(uint8_t));

    generate_matrix(img1_host, rows, columns);
    generate_matrix(img2_host, rows, columns);
    ...
    if (verbose)
        //printf("Allocating %ld MB on device\n",histos_size / (1024 * 1024));
    ...
    uint32_t *histo_host = (uint32_t*)malloc(64 * sizeof(uint32_t));

    cudaDeviceSynchronize();
    cudaMemcpy(histo_host, histo, 64 * sizeof(uint32_t), cudaMemcpyDeviceToHost);
    checkCudaError("cudaMemcpy histo_host");

    printf("Initial Values of JH: ");
    for (int i = 0; i < 32; i++) { 
        printf("%d, ", histo_host[i]); 
    }
    printf("\n");*/
